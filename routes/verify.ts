...
exports.accessControlChallenges =
  ...
  ({ url }: Request, res: Response, next: NextFunction) => {
    challengeUtils.solveIf(challenges.scoreBoardChallenge, () => {
      return utils.endsWith(url, '/1px.png')
    })
    challengeUtils.solveIf(challenges.adminSectionChallenge, () => {
      return utils.endsWith(url, '/19px.png')
    })
    challengeUtils.solveIf(challenges.tokenSaleChallenge, () => {
      return utils.endsWith(url, '/56px.png')
    })
    challengeUtils.solveIf(challenges.privacyPolicyChallenge, () => {
      return utils.endsWith(url, '/81px.png')
    })
    challengeUtils.solveIf(challenges.extraLanguageChallenge, () => {
      return utils.endsWith(url, '/tlh_AA.json')
    })
    challengeUtils.solveIf(challenges.retrieveBlueprintChallenge, () => {
      return utils.endsWith(url, cache.retrieveBlueprintChallengeFile)
    })
    challengeUtils.solveIf(challenges.securityPolicyChallenge, () => {
      return utils.endsWith(url, '/security.txt')
    })
    challengeUtils.solveIf(challenges.missingEncodingChallenge, () => {
      return utils.endsWith(
        url.toLowerCase(),
        '%f0%9f%98%bc-%23zatschi-%23whoneedsfourlegs-1572600969477.jpg'
      )
    })
    challengeUtils.solveIf(challenges.accessLogDisclosureChallenge, () => {
...
exports.errorHandlingChallenge =
  ...
  (
    ...
    challengeUtils.solveIf(challenges.errorHandlingChallenge, () => {
...
exports.jwtChallenges =
  () => (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      ...
      challengeUtils.notSolved(challenges.jwtForgedChallenge)
    ) {
...
exports.serverSideChallenges =
  () => (req: Request, res: Response, next: NextFunction) => {
    if (req.query.key === 'tRy_H4rd3r_n0thIng_iS_Imp0ssibl3') {
      if (
        challengeUtils.notSolved(challenges.sstiChallenge) &&
      ...
      if (
        challengeUtils.notSolved(challenges.ssrfChallenge) &&
...
exports.databaseRelatedChallenges =
  () => (req: Request, res: Response, next: NextFunction) => {
    if (
      challengeUtils.notSolved(challenges.changeProductChallenge) &&
      products.osaft
    ) {
    ...
    if (
      challengeUtils.notSolved(challenges.knownVulnerableComponentChallenge)
    ) {
...
function feedbackChallenge() {
  FeedbackModel.findAndCountAll({ where: { rating: 5 } })
...
function knownVulnerableComponentChallenge() {
  FeedbackModel.findAndCountAll({
    ...
    .catch(() => {
      throw new Error(
        'Unable to get data for known vulnerabilities. Please try again'
      )
    })
